...
character(len=BMI_MAX_VAR_NAME), target, dimension(output_item_count) :: &
    ...
    'soil_depth~compartment               ', &
    'soil_water_drainage~deep             ', &
    'water_runoff~surface                 ', &
...
function resolve_var_alias(name) result(canonical)
    ...
    case('soil_water__deep_percolation_flux');       canonical = 'soil_water_drainage~deep'
    case('land_surface_water__runoff_flux');         canonical = 'water_runoff~surface'
    case('soil_water__infiltration_flux');           canonical = 'soil_infiltration~rate'
    case('soil__evaporation_flux');                  canonical = 'air_evaporation'
    case('crop__transpiration_flux_actual');         canonical = 'air_transpiration~actual'
    case('soil_water__capillary_rise_flux');         canonical = 'soil_water_capillary'
    case default
        canonical = name
    end select
...
case('soil__water_content_in_layers')
    grid = 1
case('soil__water_content_in_compartments', 'soil_depth~compartment')
    grid = 2
case default
    grid = 0
end select
...
case('soil_water_actual_layer-1','soil_water_actual_layer-2','soil_water_actual_layer-3', &
     ...
     'soil__water_content_in_compartments')
    units = 'm3 m-3'
case('soil_depth~compartment')
    units = 'm'
case('soil_water_drainage~deep','water_runoff~surface', &
     'soil_infiltration~rate','air_evaporation', &
     'air_transpiration~actual','soil_water_capillary')
    units = 'mm'
...
case('soil_water_drainage~deep')
    dest(1) = real(GetDrain(), c_double)
case('water_runoff~surface')
    dest(1) = real(GetRunoff(), c_double)
case('soil_infiltration~rate')
    dest(1) = real(GetInfiltrated(), c_double)
case('air_evaporation')
    dest(1) = real(GetEact(), c_double)
case('air_transpiration~actual')
    dest(1) = real(GetTact(), c_double)
case('soil_water_capillary')
