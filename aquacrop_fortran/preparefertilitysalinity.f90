...
!! Reference-climate fertility/salinity stress-response calibration.
!!
!! Ported from official v7.3 (src/preparefertilitysalinity.f90). Two local
!! stress-level variables (SiPr in StressBiomassRelationshipForTnxReference,
!! and implicitly Si/SiPr throughout) are declared int8 rather than
!! pristine's int32, to match this fork's own established convention for
!! stress-level dummy arguments (CropStressParametersSoilFertility's
!! StressLevel, TimeToMaxCanopySF's ClassSF, and the existing sibling
!! functions StressBiomassRelationship/CCxSaltStressRelationship in
!! tempprocessing.f90 all take int8) -- not a pristine mismatch to fix, but
!! a deliberate adaptation so this new module composes with unmodified
!! existing fork code. Values involved are always in [0, 100] and lose
!! nothing in the narrower width. See docs/bmi/FORTRAN_FORK_REFERENCE_CLIMATE_FIX.md.
...
subroutine AdjustCalendarDaysReferenceTnx(PlantDayNr, TheCropType, &
    ...
    !! Ported verbatim from official v7.3 (preparefertilitysalinity.f90:94-197).
...
subroutine DailyTnxReferenceFileCoveringCropPeriod(CropFirstDay)
    !! Ported verbatim from official v7.3 (preparefertilitysalinity.f90:200-248).
    !! Writes TCropReference.SIM: the 365-day reference series re-rotated to
    !! start on CropFirstDay (the crop's own onset), and populates
    !! TminCropReferenceRun/TmaxCropReferenceRun.
...
real(dp) function CO2ForTnxReferenceYear(TnxReferenceYear)
    !! Ported verbatim from official v7.3 (preparefertilitysalinity.f90:251-291).
...
subroutine StressBiomassRelationshipForTnxReference(TheDaysToCCini, TheGDDaysToCCini,&
    ...
    !! Ported from official v7.3 (preparefertilitysalinity.f90:294-501).
    !! SiPr is declared int8 (pristine: int32) to match this fork's existing
    !! CropStressParametersSoilFertility/TimeToMaxCanopySF convention -- see
...
subroutine CCxSaltStressRelationshipForTnxReference(TheDaysToCCini, TheGDDaysToCCini,&
    ...
    !! Ported verbatim from official v7.3 (preparefertilitysalinity.f90:504-724).
    integer(int32), intent(in) :: TheDaysToCCini
    integer(int32), intent(in) :: TheGDDaysToCCini
    integer(int32), intent(in) :: L0
    integer(int32), intent(in) :: L12
    ...
    do Si = 1, 10
        ...
        BNor = Bnormalized(TheDaysToCCini, TheGDDaysToCCini,&
                ...
                int(SiPr, kind=int8), StressResponse%RedCGC, StressResponse%RedCCX,&
                StressResponse%RedWP, StressResponse%RedKsSto, &
                0_int8, 0, StressResponse%CDecline, -0.01_dp,&
                TheModeCycle, .false., .true., .false.)
        if (Si == 1) then
            BNor100 = BNor
            BioMProc = 100._dp
...
subroutine ReferenceStressBiomassRelationship(TheDaysToCCini, &
    ...
    !! Ported verbatim from official v7.3 (preparefertilitysalinity.f90:727-861).
    !! Orchestrator: builds TCropReference.SIM for this crop's onset,
    !! converts GDD-day lengths to calendar days off the reference climate
    !! when in GDD mode, resolves the reference-year CO2, then delegates to
...
subroutine ReferenceCCxSaltStressRelationship(TheDaysToCCini, &
        ...
        ThePlanting, LHImax)
    !! Ported verbatim from official v7.3 (preparefertilitysalinity.f90:864-999).
    integer(int32), intent(in) :: TheDaysToCCini
    integer(int32), intent(in) :: TheGDDaysToCCini
    integer(int32), intent(in) :: L0
    integer(int32), intent(in) :: L12
