...
subroutine OpenOutputRun(TheProjectType)
    ...
    ! Skip output if no name set
    if (len_trim(GetOutputName()) == 0) then
        return
    end if
...
! ============================================================================
! BMI Wrapper Function
! ============================================================================

!> BMI simulation wrapper - advances simulation by one day
!> This extracts the core daily loop from FileManagement() for BMI control
!>
!> @param status Output: 0 = success, 1 = simulation complete, -1 = error
subroutine BMI_SimulateOneDay(status)
    ...
    RepeatToDay = GetSimulation_ToDayNr()
    if ((GetDayNri()-1) >= RepeatToDay) then
        status = 1  ! Simulation complete
        return
    end if
    
    ! Execute one day of simulation
    ! This is the core of what FileManagement() does in its loop
    call AdvanceOneTimeStep(WPi)
    call ReadClimateNextDay()
    call SetGDDVariablesNextDay()
...
end subroutine BMI_SimulateOneDay
