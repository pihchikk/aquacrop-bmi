"""
BMI wrapper for AquaCrop with full calibration logic from sync.py
...
def suppress_fortran_output():
    """Context manager to suppress Fortran stdout/stderr"""
...
# Calibration parameters from sync.py
...
class BmiAquaCrop(Bmi):
    """
    BMI wrapper with full calibration logic from sync.py.
    
    Design:
    - Calibration happens once during initialize()
    - Results stored for all seasons/soils
    ...
    def __init__(self, original_cwd: Path | None = None) -> None:
        ...
        # Save original working directory (handle case where cwd is deleted)
        try:
            self._original_cwd = original_cwd or Path(os.getcwd())
        except FileNotFoundError:
            # If cwd is deleted, use /tmp
    ...
    def initialize(self, config_file: str) -> None:
        ...
        then initializes first season/soil combination.
        """
        ...
        # Detect type and load scenario
        self._scenario_type = self._detect_scenario_type(config)
        print(f"[BMI] Detected scenario type: {self._scenario_type}")
        
        # Perform calibration if needed
        if self._scenario_type == 'crop-calibration':
            print("[BMI] Performing crop parameter calibration...")
            ...
            print("[BMI] Performing fertility stress calibration...")
    ...
    def _initialize_season(self) -> None:
        ...
        # Get fertility stress for this season (or use single value)
        ...
        print(f"[BMI] Initializing season {self._current_season_idx + 1}/"
        ...
        print(f"[BMI] ✓ Season initialized")
    ...
    # ========================================================================
    # Internal: Scenario Detection & Conversion
    # ========================================================================
    ...
    def _convert_fertility_calibration_to_simulation(
        ...
        return ScenariosSimulationInput(
            ...
            crop_file=crop_file,
            soils=[input_data.soil],
            fertility_stress=stress_per_season[0],  # Default for get_value
        )
    ...
    # ========================================================================
    # Calibration: Crop Parameters (from sync.py)
    # ========================================================================
    ...
    def _run_crop_calibration(self, config: dict) -> dict:
        ...
        print(f"[BMI] ✓ Calibration complete: error={best_error:.4f}")
    ...
    def _param_bounds(
        ...
        """Calculate parameter bounds for sampling."""
    ...
    def _run_fertility_stress_calibration(self, config: dict) -> dict:
        ...
        # Calculate stress per season (use best stress value for all seasons initially)
        # Could be extended to vary per season
        stress_values = [int(stress_samples[best_num])] * len(input_data.seasons)
        
        print(f"[BMI] ✓ Calibration complete: error={best_error:.4f}")
        print(f"[BMI]   Best fertility stress: {stress_samples[best_num]:.0f}%")
    ...
    # ========================================================================
    # Core Calibration: Shared between crop and stress calibration
    # ========================================================================
    ...
    def _run_calibration(
        ...
        """
        Run calibration across all seasons and samples.
        
        For each season × sample combination:
        - Run simulation
        - Calculate error: |observed_yield - simulated_yield|
        
        Returns: (best_sample_index, best_error, crop_file_with_best_params)
        """
        ...
        print(f"[BMI]   Getting weather data for {date_start} to {date_end}...")
        weather_data = get_weather_data(
            point.latitude, point.longitude, point.altitude,
            date_start, date_end
        )
        
        # Generate project with common files
        with AquacropProject(with_default=False) as project:
            ...
            # Run calibration worker for each season × sample combination
            print(f"[BMI]   Running {season_count * sample_count} simulations...")
            worker = partial(
                self._calibration_worker, project.root, calibrated_params, crop_index
            )
            
            # Create all season × sample × stress combinations using product like sync.py
            args = (
        ...
        # Update crop params with best sample
    ...
    def _calibration_worker(
        ...
        try:
            with AquacropProject(parent) as project:
                ...
                # Update params with sample values
            ...
            print(f"[BMI] Calibration worker error: {e}")
            return 1e6
    ...
    def _generate_aquacrop_data(
        ...
        # Create output directory (use original working directory)
        output_dir = (self._original_cwd / "outputs").resolve()
    ...
    # ========================================================================
    # BMI: Grid Methods (not used for scalar model, but required by interface)
    # ========================================================================
