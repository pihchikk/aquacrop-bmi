#!/usr/bin/env python3
"""
Calibrate AquaCrop through BMI with step-by-step tracking

Advantage over batch:
- Interactive: see intermediate results on each day
- Dynamic: can modify parameters during calibration
- Unified: everything through BMI interface

OUTPUT FOLDER FIX:
- All outputs saved to: {input_folder}/outputs/ or {cwd}/outputs/
- With proper fallback if cwd is unavailable
"""
...
def run_bmi_simulation_for_calibration(
    ...
    Returns:
        Final yield value
    """
    # Create temp JSON with proper serialization
    temp_dict = {
...
def calibrate_through_bmi(scenario_file: Path):
    """Run full calibration through BMI interface"""
    ...
    # FIX: Save results to outputs/ folder with proper fallback
    # Try cwd first, fallback to project root (one level up from scenarios/)
    try:
        output_dir = Path(os.getcwd()) / "outputs"
    except (FileNotFoundError, OSError):
        # If cwd fails, use project root (one level above scenarios/)
        output_dir = scenario_file.parent.parent / "outputs"
